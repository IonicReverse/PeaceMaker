...
function LibDraw.GroundCircle(x, y, z, size)
	...
	for v=0, full_circle, small_circle_step do
		fx, fy, fz = lb.Raycast(  (x+cos(v)*size), (y+sin(v)*size), z+100, (x+cos(v)*size), (y+sin(v)*size), z-100, flags )
		if fx == nil then
...
function LibDraw.Texture(config, x, y, z, alphaA)
	...
	if not scale then
		local cx, cy, cz = lb.GetCameraPosition()
...
function LibDraw.DrawHelper()
	local playerX, playerY, playerZ = lb.ObjectPosition("player")
  ...
  LibDraw.SetWidth(1)
  LibDraw.Array(arrowY, playerX, playerY, playerZ, -deg45, false, false)
  LibDraw.Text('X', "GameFontNormal", playerX, playerY + 1.75, playerZ)
	-- Y
	LibDraw.SetColor(0, 255, 0, 100)
	LibDraw.SetWidth(1)
...
function LibDraw.Camera()
	local fX, fY, fZ = lb.ObjectPosition("player")
	local sX, sY, sZ = lb.GetCameraPosition()
