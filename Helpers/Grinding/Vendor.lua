...
PeaceMaker.Helpers.Grinding.Vendor = {}

local Vendors = PeaceMaker.Helpers.Grinding.Vendor
      ...
      if itemID and itemID ~= 6948 then
        ...
        if itemRarity < 4 and itemSellPrice > 0 then
          if PeaceMaker.Mode == "Grind" then
            if not Misc:CompareList(itemID, PeaceMaker.Settings.profile.Grinding.WhiteListItem) then
              UseContainerItem(BagID, BagSlot)
              Vendor.SellDelay = PeaceMaker.Time + 0.1
              break
            end
          elseif PeaceMaker.Mode == "TagGrind" then
            if not Misc:CompareList(itemID, PeaceMaker.Settings.profile.TagGrinding.WhiteListItem) then
              UseContainerItem(BagID, BagSlot)
              Vendor.SellDelay = PeaceMaker.Time + 0.1
              break
            end
    ...
    for BagSlot = 1, GetContainerNumSlots(BagID) do
      ...
      if itemID and itemID ~= 6948 then
        ...
        if itemSellPrice > 0 then
          if PeaceMaker.Mode == "Grind" then
            if Misc:CompareList(itemID, PeaceMaker.Settings.profile.Grinding.WhiteListItem) then
              Count = Count + 1
            end
          elseif PeaceMaker.Mode == "TagGrind" then
            if Misc:CompareList(itemID, PeaceMaker.Settings.profile.TagGrinding.WhiteListItem) then
              Count = Count + 1
            end
          end
        end
...
local function MailItem(MailItemList, To, Title)

  for bag = 0,4,1 do
    for slot = 1, GetContainerNumSlots(bag), 1 do
      local name = GetContainerItemLink(bag, slot)
      if name 
        and CheckMailList(MailItemList, name)
  ...
  then
    if (PeaceMaker.Settings.profile.Grinding.MailToGold or PeaceMaker.Settings.profile.TagGrinding.MailToGold) then
...
local function Core()
  ...
  local RepairVendorX, RepairVendorY, RepairVendorZ
  local RepairNpcID
  local FoodVendorX, FoodVendorY, FoodVendorZ 
  local FoodNpcID 
  local FoodID
  local DrinkID
  local FoodAmount 
  ...
  if PeaceMaker.Mode == "Grind" then
    RepairVendorX, RepairVendorY, RepairVendorZ = Misc:SplitStr(PeaceMaker.Settings.profile.Grinding.RepairNpcPos)
    RepairNpcID = tonumber(PeaceMaker.Settings.profile.Grinding.RepairNpcID)
    FoodVendorX, FoodVendorY, FoodVendorZ = Misc:SplitStr(PeaceMaker.Settings.profile.Grinding.FoodNpcPos)
    FoodNpcID = tonumber(PeaceMaker.Settings.profile.Grinding.FoodNpcID)
    FoodID = PeaceMaker.Settings.profile.Grinding.FoodID
    DrinkID = PeaceMaker.Settings.profile.Grinding.DrinkID
    FoodAmount = tonumber(PeaceMaker.Settings.profile.Grinding.FoodAmount)
    DrinkAmount = tonumber(PeaceMaker.Settings.profile.Grinding.DrinkAmount)
    UseMail = PeaceMaker.Settings.profile.Grinding.UseMail
    MailPosX, MailPosY, MailPosZ = Misc:SplitStr(PeaceMaker.Settings.profile.Grinding.MailLoc)
    Durability = PeaceMaker.Settings.profile.Grinding.Durability
    BagSlot = PeaceMaker.Settings.profile.Grinding.BagSlot
    MailItemList = PeaceMaker.Settings.profile.Grinding.MailItemList
    MountID = PeaceMaker.Settings.profile.Grinding.MountID
  end

  if PeaceMaker.Mode == "TagGrind" then
    RepairVendorX, RepairVendorY, RepairVendorZ = Misc:SplitStr(PeaceMaker.Settings.profile.TagGrinding.RepairNpcPos)
    RepairNpcID = tonumber(PeaceMaker.Settings.profile.TagGrinding.RepairNpcID)
    FoodVendorX, FoodVendorY, FoodVendorZ = Misc:SplitStr(PeaceMaker.Settings.profile.TagGrinding.FoodNpcPos)
    FoodNpcID = tonumber(PeaceMaker.Settings.profile.TagGrinding.FoodNpcID)
    FoodID = PeaceMaker.Settings.profile.TagGrinding.FoodID
    DrinkID = PeaceMaker.Settings.profile.TagGrinding.DrinkID
    FoodAmount = tonumber(PeaceMaker.Settings.profile.TagGrinding.FoodAmount)
    DrinkAmount = tonumber(PeaceMaker.Settings.profile.TagGrinding.DrinkAmount)
    UseMail = PeaceMaker.Settings.profile.TagGrinding.UseMail
    MailPosX, MailPosY, MailPosZ = Misc:SplitStr(PeaceMaker.Settings.profile.TagGrinding.MailLoc)
    Durability = PeaceMaker.Settings.profile.TagGrinding.Durability
    BagSlot = PeaceMaker.Settings.profile.TagGrinding.BagSlot
    MailItemList = PeaceMaker.Settings.profile.Grinding.MailItemList
    MountID = PeaceMaker.Settings.profile.TagGrinding.MountID
  end
...
local function CoreMammoth()
  ...
  if PeaceMaker.VendorStep == 2 then
    if (Misc:GetMaxBagSlots() - Misc:GetFreeBagSlots()) > (CountWhiteListItem + 2) then
      if not MerchantFrame:IsVisible() and not GossipFrame:IsVisible() and not Vendor.Interact then
        Misc:InteractUnit(RepairVendorID)
        Vendor.Interact = true
        C_Timer.After(1, function() Vendor.Interact = false end)
